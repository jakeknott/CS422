...
namespace CS422
{
	public class ConcatStream : Stream 
	{
		...
		public override int Read (byte[] buffer, int offset, int count)
		{
			...
			{
				bytesRead = s1.Read (buffer, offset, buffer.Length - offset);
				Position += bytesRead;
			}
			...
			// When uploading a large file, the network stream will sometimes timeout,
			// setting this to 30 second read timout will prevent this. 
			if (s2 is NetworkStream)
			{
				s2.ReadTimeout = (int) new TimeSpan(0,0,30).TotalMilliseconds;
			}

			if (count - bytesRead < buffer.Length - offset + bytesRead)
			{
			...
			{
				int bytes = s2.Read (buffer, offset + bytesRead, buffer.Length - (offset + bytesRead));
				Position += bytes;
				bytesRead += bytes;
			}
